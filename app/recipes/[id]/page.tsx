...
type StockItem = { item_id: string; name: string }
...
export default function RecipeDetail() {
  ...
  const [inStockItems, setInStockItems] = useState<StockItem[]>([])
  ...
  const load = useCallback(async () => {
    const [recipeRes, ingRes, itemsRes, stockRes] = await Promise.all([
      ...
      supabase.from('inventory_lots').select('item_id, items(name)').gt('quantity_remaining', 0),
    ])
    ...
    const stockRows = (stockRes.data ?? []) as unknown as Array<{ item_id: string; items: { name: string } }>
    setInStockIds(new Set(stockRows.map(r => r.item_id)))
    const seen = new Set<string>()
    const deduped: StockItem[] = []
    for (const r of stockRows) {
      if (!seen.has(r.item_id)) {
        seen.add(r.item_id)
        deduped.push({ item_id: r.item_id, name: r.items.name })
      }
    }
    setInStockItems(deduped)
    setLoading(false)
  }, [id])

  useEffect(() => { load() }, [load])

  function getPartialMatches(ingredientName: string, ingredientItemId: string): string[] {
    const a = ingredientName.toLowerCase()
    return inStockItems
      .filter(s => {
        if (s.item_id === ingredientItemId) return false
  ...
  async function handleCreateItem(name: string) {
    ...
    const { data, error } = await supabase
      .from('items')
      .insert({ name: name.trim(), category: 'grocery' })
      .select()
      .single()
    if (error) { setError(error.message); setCreatingItem(false); return }
    const newItem: Item = { id: data.id, name: data.name, brand: null }
    setAllItems(prev => [...prev, newItem].sort((a, b) => a.name.localeCompare(b.name)))
    setIngItemId(data.id)
      ...
      <div className="p-4 flex flex-col gap-4 pb-10">
        ...
        {editing ? (
          <div className="bg-white rounded-xl shadow-sm p-4 flex flex-col gap-3">
            ...
            <textarea value={editDesc} onChange={e => setEditDesc(e.target.value)} placeholder="Description, instructions, notes…" rows={6} className="input text-sm" />
          ...
          <div className="bg-white rounded-xl shadow-sm p-4">
            {recipe?.description && <p className="text-sm text-gray-600 mb-2 whitespace-pre-wrap">{recipe.description}</p>}
        ...
        <div>
          ...
          <div className="flex flex-col gap-1">
            {ingredients.map(ing => {
              ...
              const partialMatches = !inStock ? getPartialMatches(ing.items.name, ing.item_id) : []
              ...
              return (
                <div key={ing.item_id} className="bg-white rounded-lg px-3 py-2.5 flex items-start gap-3 shadow-sm">
                  <span className={`w-2 h-2 rounded-full shrink-0 mt-1.5 ${
                    inStock ? 'bg-green-500' : hasPartial ? 'bg-amber-400' : 'bg-gray-300'
                  }`} />
                  <div className="flex-1 min-w-0">
                    <p className={`text-sm ${inStock ? 'text-gray-900' : hasPartial ? 'text-gray-700' : 'text-gray-400'}`}>{label}</p>
                    {hasPartial && (
                      <p className="text-xs text-amber-600 mt-0.5 truncate">{partialMatches.join(', ')} available</p>
                    )}
                  </div>
                  {!inStock && (
                    <button
                      onClick={() => router.push(`/add?item_id=${ing.item_id}&item_name=${encodeURIComponent(ing.items.name)}`)}
                      className="p-1 text-gray-300 hover:text-green-600 active:text-green-800 transition-colors shrink-0"
                  ...
                  <button
                    ...
                    className="p-1 text-gray-300 hover:text-red-400 active:text-red-600 transition-colors disabled:opacity-30 shrink-0"
              ...
              <div className="flex flex-col gap-1">
                ...
                {itemSearch && !ingItemId && (
                  <div className="border border-gray-200 rounded-lg overflow-hidden max-h-48 overflow-y-auto">
                    ...
                    <button
                      type="button"
                      onClick={() => handleCreateItem(itemSearch)}
                      disabled={creatingItem}
                      className="w-full text-left px-3 py-2 text-sm text-green-700 font-semibold hover:bg-green-50 active:bg-green-100 border-t border-gray-200 flex items-center gap-1.5 disabled:opacity-50"
                    >
                      <svg xmlns="http://www.w3.org/2000/svg" className="w-3.5 h-3.5 shrink-0" fill="none" viewBox="0 0 24 24" stroke="currentColor">
                        <path strokeLinecap="round" strokeLinejoin="round" strokeWidth={2.5} d="M12 4v16m8-8H4" />
                      </svg>
                      {creatingItem ? 'Creating…' : `Create "${itemSearch}"`}
                    </button>
                  </div>
                )}
                {ingItemId && (
        ...
        {ingredients.length > 0 && (
          <div className="flex items-center gap-4 text-xs text-gray-400 px-1 flex-wrap">
            ...
            <span className="flex items-center gap-1.5"><span className="w-2 h-2 rounded-full bg-amber-400" /> Variant available</span>
